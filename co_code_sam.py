# to check if the register value becomes 0 when overflow is there and fag ka jo required bit is toggled
...
def apply(i):
    global ErrorFlag
    global dictreg    #change 1
    k = i.split(" ")
    string = ""

    # seperate conditions for mov immediate and mov register
    if len(k)>1:
        if k[2]=="FLAG":
            try:
                assert(k[0]=="mov" & k[1] in register)
                dictreg[k[1]]=flag
            except AssertionError:
