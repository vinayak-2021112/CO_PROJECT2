...
def immediate_check(i):
    ...
    if register_count == 1:
        try:
            ...
            ErrorArray.append(f"ERROR: Immediate value at line {line_counter + 1} is more than 8 bits")
            ErrorFlag += 1
...
def apply(i):
    ...
    if len(k)>1:
        if k[1]=="FLAG":
            try:
                assert(k[0]=="mov" & k[2] in register)
                dictreg[k[2]]=flag
            except AssertionError:
    ...
    if k[0] in ["div", "not", "cmp"]:
        ...
        # if k[0]=="not":
        #     #to be done
        #     dictreg[k[2]]=invert(binary(dictreg[k[1]])) 
        #     print("to be done")
        if k[0] == "cmp":
