try:
    ...
    def apply(i):
        ...
        for u in k:
            if(list(u)==[]):
                ErrorArray.append(f"ERROR: Incorrect instruction Syntax at line {line_counter}")
                ErrorFlag+=1
                return
        ...
        if k[0] == "var":
            try:
                ...
                ErrorArray.append(f"ERROR: Incorrect variable syntax at line {line_counter}")
                ErrorFlag+=1
                return
    ...
    #flag error handling
        if len(k)>1:
            if "FLAGS" in k:
                ...
                try:
                    ...
                    ErrorArray.append(f"ERROR: Illegal use of flag register at line {line_counter}")
                    ErrorFlag += 1
                    return 
        ...
        if k[0] == "mov" and k[2] not in register.keys():
            try:
                ...
                ErrorArray.append(f"Error: Immediate value has incorrect syntax at line {line_counter}")
                ErrorFlag+=1
                return
        ...
        if k[0] in ["add", "sub", "mul", "xor", "or", "and"]:
            try:
                ...
                ErrorArray.append(f"ERROR: Insufficient Registers at line {line_counter}")
                ErrorFlag+=1
                addtoLabel=False
                return
        ...
        if k[0] in ["ls", "rs"]:
            if k[1] not in reg.keys():
                ErrorArray.append(f"ERROR:Invalid instruction syntax at line {line_counter}")
            try:
                assert int(k[2][1:]) <= 255
    ...
    def check_labels(i):
        ...
        if k[0] == "mov" or k[0] == "var":
            ...
            if k[0][-1] == ":":
                if len(k)==1:
                    ErrorFlag+=1
                    ErrorArray.append(f"ERROR: No instruction after label at line {line_counter}")
                    return -1
                label_len = len(k[0])
                if (i[label_len] == " ") and (i[label_len + 1] != " "):
                    if k[1] in instruction.keys() or k[1] == "mov":
                        ...
                        ErrorFlag += 1
                        ErrorArray.append(f"ERROR: Invalid label syntax at line {line_counter}")
                        return -1
                else:
                    ErrorFlag += 1
                    ErrorArray.append(f"ERROR: Invalid label syntax at line {line_counter}")
